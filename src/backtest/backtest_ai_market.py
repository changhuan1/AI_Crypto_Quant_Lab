...
from src.jobs.ai_market_update import build_ai_market_features, load_ai_market_prices
from src.strategies.ai_market_scoring import calculate_ai_market_score
from src.strategies.ai_rotation import generate_ai_targets


DB_PATH = Path("data/db/quant_lab.duckdb")
STRATEGY_NAME = "ai_market_rotation_v0"
...
def generate_weekly_ai_signals(scores: pd.DataFrame) -> pd.DataFrame:
    records = []
    ...
    for date, frame in scores.groupby("date", sort=True):
        ...
        regime = classify_ai_market_risk_regime(scores.loc[scores["date"] <= date])
        targets = generate_ai_targets(frame, regime)

        for asset_id, weight in targets.items():
            if asset_id == "LOW_RISK_FUND":
                continue
            ...
            records.append(
                {
                    ...
                    "reason": f"ai_market_regime={regime}",
...
def _score_for_asset(scores: pd.DataFrame, asset_id: str) -> float | None:
    match = scores.loc[scores["asset_id"] == asset_id, "ai_market_score"]
...
def main() -> None:
    ...
    scores = calculate_ai_market_score(features)
    signals = generate_weekly_ai_signals(scores)
    ...
    print(f"AI market signals saved: {len(signals)} rows")
