...
def load_crypto_prices(db_path: Path = DB_PATH) -> pd.DataFrame:
    with duckdb.connect(str(db_path)) as con:
        return con.execute(
            """
            SELECT *
            FROM prices_daily
            WHERE asset_id LIKE 'CRYPTO_%'
            ORDER BY asset_id, date
            """
        ).df()


def build_crypto_features(prices: pd.DataFrame) -> pd.DataFrame:
    features = add_price_features(prices)
    return add_relative_strength(features, benchmark_asset_id="CRYPTO_BTC_USDT", window=20)
...
def main() -> None:
    ...
    regime = classify_btc_risk_regime(features)
    print(f"Crypto features saved: {len(feature_rows)} rows")
    print(f"BTC risk regime: {regime}")
